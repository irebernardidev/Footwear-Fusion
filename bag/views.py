    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request,
                    f'The quantity of {product.name} is updated'
                )
            else:
                bag[item_id]['items_by_size'][size] = quantity
                messages.success(
                    request,
                    f'{product.name} size {size.upper()} is added to your bag'
                )
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(
        ...
        if item_id in list(bag.keys()):
            ...
            messages.success(
                request,
                f'The quantity of {product.name} is updated in your bag'
            )
        else:
            bag[item_id] = quantity
...
def remove_from_bag(request, item_id):
    ...
    try:
        ...
        if size:
            ...
            messages.success(
                request,
                f'{product.name} size {size.upper()} is removed from your bag'
            )
            ...
            messages.success(
                request,
                f'{product.name} is removed from your bag'
            )
